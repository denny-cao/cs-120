    ...
    for node in range(G.N):
        if node not in visited:
            ...
            while queue:
                ...
                curr_color = G.colors[curr_node]

                for neighbor in G.edges[curr_node]:
                    if G.colors[neighbor] is None:
                        queue.append(neighbor)
                        visited.add(neighbor)
                        G.colors[neighbor] = 1 - curr_color
                        
                    elif G.colors[neighbor] == curr_color:
                        G.reset_colors()
                        return None
    return G.colors
...
def iset_bfs_3_coloring(G):
    ...
    '''
    Lemma 2.1:
    For a graph G = (V,E) and S = \subseteq V, let let G_{-S} = (V-S, E_{-S}) where E_{-S} = {{u,v} \in E: u,v \not\in S}.

    Then:
    1. If G = (V,E) is 3-colorable, then there is an independent set S \subseteq V of size at most n/3 such that G_{-S} is 2-colorable.
    2. If for some independent set S \subseteq V, G_{-S} is 2-colorable, then G is 3-colorable. Moreover, if f_{-S}: V - S \rightarrow {0,1} is a 2-coloring of G_{-S}, then a 3-coloring f of G is given by:
        f(v) = \begin{cases}
            2 & v \in S \\
            f_{-S}(v) & v \not\in S
        \end{cases}

    Pseudocode for 3by2Coloring(G)
    Input: A graph G = (V,E)
    Output: A (proper) 3-coloring f of G, or \perp if none exists

    for each S \subseteq V of size at most n/3 do
        if S is an independent set in G then
            Construct the graph G_{-S} as defined in Lemma 2.1;
            Let f_{-S} = 2-Coloring(G_{-S});
            if f_{-S} != \perp then
                Construct a 3-coloring f from f_{-S} as in Lemma 2.1;
                return f
    return \perp
    '''
    ...
    for size in range(n//3 + 1):
        for S in combinations(range(n), size):
            if is_independent_set(G, S):
                ...
                for node in S:
                    for neighbor in G.edges[node]:
                        G_minus_S.remove_edge(node, neighbor) 
                
                # 2-color G_{-S}
                f_minus_S = bfs_2_coloring(G_minus_S)
                
                if f_minus_S is not None:
                    # Construct 3-coloring f from f_{-S}
                    return [2 if v in S else f_minus_S[v] for v in range(n)]
