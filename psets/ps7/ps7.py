...
def sat_3_coloring(G):
    ...
    # Add clauses for each node having at least one color
    for v in range(G.N):
            solver.add_clause([3*v + 1, 3*v + 2, 3*v + 3]) # v is colored 1 or 2 or 3
    
    # Add clauses for each node having at most one color
    for v in range(G.N):
        for c1 in range(1, 4):
            for c2 in range(c1 + 1, 4):
                solver.add_clause([-3*v - c1, -3*v - c2]) # v is not colored c1 and c2
    
    # Add clauses for each edge having different colors
    for u in range(G.N):
        for v in G.edges[u]:
            for c in range(1, 4):
                solver.add_clause([-3*u - c, -3*v - c]) # u and v are not colored c
    ...
    # Convert the solution into a coloring
    for v in range(G.N):
        for c in range(1, 4):
            if 3*v + c in solution:
                G.colors[v] = c
                break
